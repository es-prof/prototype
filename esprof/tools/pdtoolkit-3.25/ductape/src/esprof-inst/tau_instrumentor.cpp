...
void processExitOrAbort(vector<itemRef *>& itemvec, const pdbItem *rit, pdbRoutine::callvec & c)
{
  for (pdbRoutine::callvec::iterator cit = c.begin(); cit != c.end(); cit++) {
    ...
    if (rr->parentGroup() == (const pdbGroup*)NULL) {
      ...
      } else // using exit keyword

      if (rr->name() == "abort") { // routine calls abort
        itemvec.push_back(new itemRef(rit, EXIT, (*cit)->line(), (*cit)->col()));
      } else { // normal callees -- ertl-liyixiao
        auto prototype = rr->fullName().substr(0, rr->fullName().length() - 2);
        itemvec.push_back(new itemRef((pdbItem *)NULL, FUNCTION_CALL, (*cit)->line(), 1, prototype, BEFORE));
        itemvec.push_back(new itemRef((pdbItem *)NULL, FUNCTION_CALL, (*cit)->line(), 2, prototype, AFTER)); 
      }
    }
  }
}
...
bool instrumentCXXFile(PDB& pdb, pdbFile* f, string& outfile, string& group_name, string &header_file)
{
  ...
  // TODO: here is hack -- ertl-liyixiao
    test_call_python(itemvec);

  // Begin Instrumentation
#if 1 // TODO: use header_snippet instead of header_file -- ertl-liyixiao
    ostr << header_snippet << endl;
#else
  ...
  for (vector<itemRef *>::iterator it = itemvec.begin(); it != itemvec.end(); ++it) {
    ...
    while (!instrumented && getInputLine()) {
      ...
      case FUNCTION_CALL: { // add -- ertl-liyixiao
        assert((*it)->attribute == BEFORE);
...
#endif /* DEBUG */
      filename = argv[2];
      esprof_filename = string(realpath(filename, NULL));
      break;
    default:
