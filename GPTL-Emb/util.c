...
#else
    fprintf (stderr, "GPTLerror: no vfprintf: fmt is %s\n", fmt);
#endif
    if (num_errors == max_errors)
      fprintf (stderr, "Truncating further error print now after %d msgs",
		      num_errors);
  }
...
void GPTLwarn (const char *fmt, ...)
{
  ...
  if (fmt != NULL && num_warn < max_warn) {
    ...
    (void) vfprintf (stderr, fmt, args);
#else
    fprintf (stderr, "GPTLwarning: no vfprintf: fmt is %s\n", fmt);
#endif
    if (num_warn == max_warn)
      fprintf (stderr, "Truncating further warning print now after %d msgs",
		      num_warn);
  }
...
void GPTLnote (const char *fmt, ...)
{
  ...
  if (fmt != NULL) {
    ...
    (void) vfprintf (stderr, fmt, args);
#else
    fprintf (stderr, "GPTLnote: no vfprintf: fmt is %s\n", fmt);
#endif
  }
